...
ok = f"{get('emotion/ok.png').cqcode}"
...
async def set_admin(bot, event):
    ...
    gid = event.group_id
    sid = event.self_id
    owner = await bot.get_group_member_info(user_id=sid, group_id=gid, no_cache = True)
    role = owner['role']
    if role == 'member':
        await bot.send(event, '冰祈不是群主哦' + emmm)
        return
    ...
    if event.user_id == SUPERUSERS[0]:
        ...
        await bot.send(event, '不可以这样做' + no)
...
async def set_group_name(bot, event):
    ...
    if not name:
        await bot.finish(event, '???是皇帝的群名嘛?')
    sid = event.self_id
    owner = await bot.get_group_member_info(user_id=sid, group_id=gid, no_cache = True)
    role = owner['role']
    if role == 'member':
        await bot.send(event, '冰祈不是管理员啦...' + emmm)
        return
    u_priv = priv.get_user_priv(event)
...
async def unban_user(bot, event):
    ...
    if role == 'member':
        ...
        return
    try:
        await bot.set_group_ban(group_id=gid, user_id=unban_uid, duration=0)
        await bot.send(event, ok)
    except Exception as ex:
        hoshino.logger.error(ex)
...
async def set_group_title(bot, ctx):
    ...
    role = owner['role']
    if role != 'owner':
        await bot.send(ctx, '冰祈不是群主哦' + emmm)
        return
    try:
        await bot.set_group_special_title(group_id=group, user_id=uid, special_title=msg, duration=-1)
        await bot.send(ctx, f'头衔{msg}设置完成' + ok)
    except Exception as ex:
        sv.logger.error(ex)
        await bot.send(ctx, f'申请头衔{msg}失败了' + sorry)
...
async def kuangwo(bot, event):
    ...
    txt = requests.get(url=url, timeout=20).text
...
async def txt_to_voice(bot, event):
    ...
    try:
        ...
        await bot.send(event, f"冰祈学不动了...{get('emotion/nani.png').cqcode}")
